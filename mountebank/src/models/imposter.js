...
async function create (Protocol, creationRequest, baseLogger, config, isAllowedConnection) {
    ...
    async function getResponseFor (request, requestDetails) {
        // Handle case where request might be undefined or missing ip property
        if (!request) {
            logger.warn('getResponseFor called with undefined request object');
            request = { ip: 'unknown' };
        }
        
        const requestIp = request.ip || 'unknown';
        
        if (!isAllowedConnection(requestIp, logger)) {
