...
function create (protocols, imposters, logger, allowInjection) {
    ...
    async function postRequest (request, response) {
        try {
            ...
            // Ensure request.body.request exists and has required properties
            const requestBody = request.body.request || request.body;
            
            // Add IP address from the HTTP request if not present
            if (!requestBody.ip && request.ip) {
                requestBody.ip = request.ip;
            }
            
            // Add connection info if not present
            if (!requestBody.ip && request.connection && request.connection.remoteAddress) {
                requestBody.ip = request.connection.remoteAddress;
            }
            
            // Fallback to localhost if still no IP
            if (!requestBody.ip) {
                requestBody.ip = '127.0.0.1';
            }
            
            const jsonResponse = await imposter.getResponseFor(requestBody);

            response.send(jsonResponse);
            
        } catch (error) {
            logger.error('Error in postRequest:', error);
